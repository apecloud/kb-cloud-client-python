...
@dataclass
class ApikeyList:
    """APIKeys is a list of APIKey objects"""
    # Required fields
    items: List[Apikey]
    # Optional fields
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ApikeyList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[Apikey.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
