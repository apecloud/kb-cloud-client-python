...
@dataclass
class BackupRepoList:
    """BackupRepoList is a list of backupRepo"""
    # Required fields
    items: List[BackupRepo]
    # Optional fields
    page_result: Optional[PageResult] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "BackupRepoList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[BackupRepo.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
