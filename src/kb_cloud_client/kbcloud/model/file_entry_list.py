...
@dataclass
class FileEntryList:
    """BackupEntryList is a list of entry"""
    # Required fields
    # Optional fields
    items: Optional[List[FileEntry]] = None
    page_result: Optional[PageResult] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "FileEntryList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[FileEntry.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
