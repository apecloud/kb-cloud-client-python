...
@dataclass
class DisasterRecoveryClusterList:
    """disasterRecoveryClusterList is a list of disaster recovery clusters"""
    # Required fields
    items: List[DisasterRecoveryClusterItem]
    # Optional fields
    page_result: Optional[PageResult] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "DisasterRecoveryClusterList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[DisasterRecoveryClusterItem.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
