# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at ApeCloud (https://www.apecloud.com/).
# Copyright 2022-Present ApeCloud Co., Ltd
# This file is auto-generated. Do not edit manually.
from __future__ import annotations

from dataclasses import dataclass, field
from datetime import datetime
from typing import Any, Dict, List, Optional, Union

from ...api_client import _parse_datetime  # noqa: F401 – used in generated from_dict

from .component_version_architecture import ComponentVersionArchitecture


@dataclass
class EngineServiceVersionsItem:
    """EngineServiceVersionsItem"""
    # Required fields
    # Optional fields
    default: Optional[bool] = None
    default_minor_version: Optional[str] = None
    major_version: Optional[str] = None
    minor_versions: Optional[List[str]] = None
    architecture: Optional[ComponentVersionArchitecture] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "EngineServiceVersionsItem":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            default=data.get("default"),
            default_minor_version=data.get("defaultMinorVersion"),
            major_version=data.get("majorVersion"),
            minor_versions=data.get("minorVersions"),
            architecture=ComponentVersionArchitecture.from_dict(data.get("architecture")) if isinstance(data.get("architecture"), dict) else data.get("architecture"),
        )

    def to_dict(self) -> Dict[str, Any]:
        """Serialize to a dict, omitting None values."""
        result: Dict[str, Any] = {}
        if self.default is not None:
            _v = self.default
            result["default"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.default_minor_version is not None:
            _v = self.default_minor_version
            result["defaultMinorVersion"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.major_version is not None:
            _v = self.major_version
            result["majorVersion"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.minor_versions is not None:
            _v = self.minor_versions
            result["minorVersions"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.architecture is not None:
            _v = self.architecture
            result["architecture"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        return result
