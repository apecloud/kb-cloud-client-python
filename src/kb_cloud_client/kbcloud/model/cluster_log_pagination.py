# Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
# This product includes software developed at ApeCloud (https://www.apecloud.com/).
# Copyright 2022-Present ApeCloud Co., Ltd
# This file is auto-generated. Do not edit manually.
from __future__ import annotations

from dataclasses import dataclass, field
from datetime import datetime
from typing import Any, Dict, List, Optional, Union

from ...api_client import _parse_datetime  # noqa: F401 – used in generated from_dict



@dataclass
class ClusterLogPagination:
    """Cluster log pagination is the pagination of the cluster log"""
    # Required fields
    # Optional fields
    end_time: Optional[str] = None
    limit: Optional[str] = None
    next_end_time: Optional[str] = None
    next_start_time: Optional[str] = None
    start_time: Optional[str] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ClusterLogPagination":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            end_time=data.get("endTime"),
            limit=data.get("limit"),
            next_end_time=data.get("nextEndTime"),
            next_start_time=data.get("nextStartTime"),
            start_time=data.get("startTime"),
        )

    def to_dict(self) -> Dict[str, Any]:
        """Serialize to a dict, omitting None values."""
        result: Dict[str, Any] = {}
        if self.end_time is not None:
            _v = self.end_time
            result["endTime"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.limit is not None:
            _v = self.limit
            result["limit"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.next_end_time is not None:
            _v = self.next_end_time
            result["nextEndTime"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.next_start_time is not None:
            _v = self.next_start_time
            result["nextStartTime"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        if self.start_time is not None:
            _v = self.start_time
            result["startTime"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
        return result
