...
@dataclass
class UserOrgList:
    """UserOrgList is a list of UserOrgs"""
    # Required fields
    items: List[UserOrg]
    # Optional fields
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "UserOrgList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[UserOrg.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
