...
@dataclass
class ClusterExecutionLog:
    """Cluster execution log is the execution log of the cluster"""
    # Required fields
    # Optional fields
    items: Optional[List[ClusterExecutionLogItem]] = None
    pagination: Optional[ClusterLogPagination] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ClusterExecutionLog":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[ClusterExecutionLogItem.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
