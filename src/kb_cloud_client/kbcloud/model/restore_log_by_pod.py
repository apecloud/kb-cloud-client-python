...
@dataclass
class RestoreLogByPod:
    """info of restore workload log"""
    # Required fields
    # Optional fields
    log: Optional[str] = None
    owner_object_key: Optional[str] = None
    pod_name: Optional[str] = None
    creation_timestamp: Optional[datetime] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "RestoreLogByPod":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            log=data.get("log"),
            owner_object_key=data.get("ownerObjectKey"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.owner_object_key is not None:
            _v = self.owner_object_key
            result["ownerObjectKey"] = _v.to_dict() if hasattr(_v, "to_dict") else (
                [i.to_dict() if hasattr(i, "to_dict") else i for i in _v] if isinstance(_v, list) else _v
            )
