...
@dataclass
class AlertRuleList:
    """AlertRuleList is a list of alert rules"""
    # Required fields
    items: List[AlertRuleGroup]
    # Optional fields
    page_result: Optional[PageResult] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "AlertRuleList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[AlertRuleGroup.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
