...
@dataclass
class NodeScaleRequest:
    """NodeScaleRequest"""
    # Required fields
    nodes: List[NodeScaleSpec]
    # Optional fields
    masters: Optional[List[SshNodeSpec]] = None
    default_ssh: Optional[SshConfig] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "NodeScaleRequest":
        ...
        return cls(
            ...
            nodes=[NodeScaleSpec.from_dict(i) if isinstance(i, dict) else i for i in data.get("nodes")] if data.get("nodes") is not None else None,
