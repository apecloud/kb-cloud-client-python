...
@dataclass
class ParamTplApplyToClusterList:
    """paramTplAPPLToClusterList is a list of parameter templates applicable to the cluster"""
    # Required fields
    items: List[ParamTplApplyToClusterListItem]
    # Optional fields
    page_result: Optional[PageResult] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ParamTplApplyToClusterList":
        """Deserialize from a dict, recursively converting nested models."""
        if data is None:
            return None
        return cls(
            items=[ParamTplApplyToClusterListItem.from_dict(i) if isinstance(i, dict) else i for i in data.get("items")] if data.get("items") is not None else None,
